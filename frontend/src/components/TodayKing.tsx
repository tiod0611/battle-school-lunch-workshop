...
function TodayKing() {
  ...
  return (
    <section className="today-king-box" aria-labelledby="today-king-title">
      ...
      {loading ? (
        <p className="status-message">토너먼트 집계 중입니다...</p>
      ) : null}
      {!loading && notReady ? (
      ...
      {!loading && !error && !notReady && data ? (
        <>
          ...
          <p className="king-score">
            건강점수 <b>{data.champion.score.total}점</b> (전국{" "}
            {data.totalParticipants}개교 중 1위)
          </p>
          ...
          <p className="king-comment">
            "{data.champion.schoolName} 급식, 오늘은 알고리즘도 두 손
            들었습니다."
          </p>
