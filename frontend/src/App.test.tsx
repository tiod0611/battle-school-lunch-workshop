...
const server = setupServer(
  http.get(`${apiBaseUrl}/api/tournament/today`, () =>
    HttpResponse.json(todayKingResponse),
  ),
  http.get(`${apiBaseUrl}/api/schools/search`, () =>
    HttpResponse.json(schoolResponse),
  ),
...
describe("급식배틀 프론트엔드", () => {
  ...
  it("잘못된 날짜 범위면 오류를 보여주고 API 호출을 막는다", async () => {
    ...
    await user.click(screen.getByRole("button", { name: "검 색" }));
    await user.click(
      await screen.findByRole("button", { name: /서울OO고등학교/ }),
    );
  ...
  it("급식 결과 정상/빈결과/API 오류를 각각 표시한다", async () => {
    ...
    expect(
      await screen.findByRole("table", { name: "급식 조회 결과" }),
    ).toBeInTheDocument();
    ...
    await user.click(screen.getByRole("button", { name: "급식 조회" }));
    expect(
      await screen.findByText("일시적인 급식 조회 오류입니다."),
    ).toBeInTheDocument();
  });
  ...
  it("오늘의 왕 정상 표시와 404 준비중 상태를 처리한다", async () => {
    ...
    server.use(
      http.get(`${apiBaseUrl}/api/tournament/today`, () =>
        HttpResponse.json(
          { detail: "오늘의 왕 준비 중", code: "TOURNAMENT_NOT_READY" },
          { status: 404 },
        ),
      ),
    );
    ...
    expect(
      await screen.findByText(
        "오늘의 왕 준비 중! 배틀 심사위원이 아직 점심을 먹는 중입니다.",
      ),
    ).toBeInTheDocument();
  });
});
