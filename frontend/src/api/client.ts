...
async function requestJson<T>(path: string, params?: Record<string, string | number | undefined>) {
  ...
  if (!response.ok) {
    const detail =
      body && typeof body.detail === 'string' ? body.detail : '요청 처리 중 오류가 발생했습니다.'
...
export function searchSchools(keyword: string, page = 1, limit = 20) {
  return requestJson<SchoolSearchResponse>('/api/schools/search', { keyword, page, limit })
}

export function getMeals(schoolCode: string, officeCode: string, from: string, to: string) {
  return requestJson<MealSearchResponse>('/api/meals', { schoolCode, officeCode, from, to })
}
