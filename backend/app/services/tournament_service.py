...
OFFICE_CODES = ["B10", "C10", "D10", "E10", "F10", "G10", "H10", "I10", "J10", "K10", "L10", "M10", "N10", "P10", "Q10", "R10", "S10"]
...
def draw_schools_for_today(target_date: date | None = None, rng: random.Random | None = None) -> list[dict]:
    ...
    with ThreadPoolExecutor(max_workers=MEAL_LOOKUP_MAX_WORKERS) as executor:
        for office_code in OFFICE_CODES:
            ...
            office_selected = _select_for_office(office_code, quotas[office_code], target_date, remaining_pool, executor)
            selected.extend(office_selected)

        if len(selected) < 128:
            selected.extend(_fill_from_other_offices(128 - len(selected), target_date, remaining_pool, rng, executor))
...
def _fetch_meals_batch(
    ...
    """급식 조회를 배치 단위로 동시 실행해 학교 하나씩 순차 호출하는 것보다 훨씬 빠르게 결과를 모은다."""
    futures = {
        executor.submit(
...
def build_bracket(schools: list[dict]) -> list[TournamentRound]:
    ...
    current = schools[:]
    round_number = 1
    total_rounds = int(math.log2(len(current))) if current else 0
        ...
        for index in range(0, len(current), 2):
            ...
            matches.append(
                TournamentMatch(
                    ...
                    schoolB=_to_tournament_school(school_b) if school_b else None,
                    winnerSchoolCode=winner["schoolCode"],
                    isBye=school_b is None,
                )
            )
        rounds.append(TournamentRound(roundNumber=round_number, roundName=round_name, matches=matches))
        current = winners
        round_number += 1
    return rounds


def _to_tournament_school(candidate: dict | None) -> TournamentSchool | None:
    if candidate is None:
        return None
    payload = {key: candidate[key] for key in ("schoolCode", "schoolName", "officeName", "menuItems", "score")}
    return TournamentSchool.model_validate(payload)


def run_daily_tournament(db: Session, run_date: date | None = None, rng: random.Random | None = None) -> TodayKingResponse:
    ...
    champion = bracket[-1].matches[0].schoolA if bracket[-1].matches[0].winnerSchoolCode == bracket[-1].matches[0].schoolA.schoolCode else bracket[-1].matches[0].schoolB
